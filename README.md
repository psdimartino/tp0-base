...
Se crea el archivo ejecutable `generar-compose.sh`. Dicho archivo tiene como única función realizar correctamente la llamada
al script de python `docker-compose-generator.py` con primer argumento el nombre del archivo de salida y como segundo argumento la cantidad de clientes.
...
```
Puede ser necesario obtener los permisos para ejecutar el archivo, para esto se debe correr
```shell
sudo chmod +x ./generar-compose.sh
```
